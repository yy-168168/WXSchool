					...
					<table width="100%" cellpadding="0" cellspacing="0" id="sortTable">
						...
						List<OperateRecord> operateRecords = (List<OperateRecord>)request.getAttribute("operateRecords");
							for (int i = 0; operateRecords != null && i < operateRecords.size(); i++) {
								...
								String imgUrl = operateRecord.getHeadImgUrl();
						...
						<tr align="center">
							<td>
								<img width='50px' alt="" src="<%=imgUrl_46%>" onclick="bigImg(this, '<%=token %>', '<%=wxaccount %>', '<%=operateRecord.getUserwx() %>')">
							</td>
							<td>
								<a target="_blank" href="/mngs/operate?ac=listru&token=<%=token%>&wxaccount=<%=wxaccount %>&userwx=<%=operateRecord.getUserwx() %>"><%=operateRecord.getNickname() %></a>
							</td>
							<td>
