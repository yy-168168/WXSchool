...
<script type="text/javascript">
	function insertBlack(userwx, cate){
		var url = "/mngs/black?ac=add&token=<%=token%>&wxaccount=<%=wxaccount %>&userwx="+userwx+"&cate="+cate;
					...
					<table width="100%" cellpadding="0" cellspacing="0" id="sortTable">
						...
						List<ChatRecord> chatRecords = (List<ChatRecord>)request.getAttribute("chatRecords");
							for (int i = 0; chatRecords != null && i < chatRecords.size(); i++) {
								...
								String imgUrl = chatRecord.getWxUser().getHeadImgUrl();
						...
						<tr align="center">
							<td>
								<img width='50px' alt="" src="<%=imgUrl_46%>" onclick="bigImg(this, '<%=token %>', '<%=wxaccount %>', '<%=chatRecord.getFrom() %>')">
							</td>
							<td align="left">
								<%=chatRecord.getContent() %>
							</td>
							<td>
								<%=chatRecord.getAddTime() %>
							</td>
							<td>
								<a href="javascript:void(-1);">删除</a>
								<a href="javascript:insertBlack('<%=chatRecord.getFrom()%>', 1)">拉黑</a>
							</td>
						</tr>
						<%
							}
