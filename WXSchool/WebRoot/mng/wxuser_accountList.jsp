...
function getUserListFromWX(wxaccount){
	var isConfirm = window.confirm("风险提示：\n该操作将从微信接口获取所有关注用户的微信资料并插入数据库，会消耗大量时间，请谨慎操作");
	if(isConfirm){
		var openId = document.getElementsByName("openId")[0].value;
		var url = "/mngs/user?ac=getUserListFromWX&token=<%=token%>";
		$.get(url,{
			wxaccount: wxaccount,
			openId: openId
			},function(data){
				...
				<div class="list">
					<table width="100%" cellpadding="0" cellspacing="0" id="sortTable">
						...
						<tr align="center">
							...
							<td>
								<a href="/mngs/user?ac=listu&token=<%=token%>&wxaccount=<%=accounts[i][1]%>">查看用户</a>
							</td>
							<td>
								<input type="text" name="openId" value="" class="input_text" size="50" style="width: 170px">
								<br />
								<a href="javascript:getUserListFromWX('<%=accounts[i][0]%>');">获取用户</a>
							</td>
						</tr>
						<%
							}
