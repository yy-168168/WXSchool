...
function check() {
	...
	var url = "mobileT/edun?ac=login&wxaccount=<%=wxaccount%>&userwx=<%=userwx%>";
	$.post(url, {
			username : username,
			password : password,
			code : code
		...
		function(data) {
			...
			if (obj == "ok") {// 登录成功，业务判断
				if ('<%=bns %>' == 'score') {// 成绩
					location.href = "mobileT/edun?ac=score&wxaccount=<%=wxaccount%>&userwx=<%=userwx%>";
				}
			} else {
				if (obj == "wrong") {
					obj = "网络连接失败";
				...
				document.getElementById("validateCode").src = "mobileT/edun?ac=getValidateCode&wxaccount=<%=wxaccount%>&t="
						+ Math.random();
			}
		...
		<form method="post">
			<div class="html5yj">
				<div class="formhead_n">
					<div>
						<span class="glyphicon glyphicon-edit"></span>&nbsp;教务平台登录
					</div>
					<div>
						1.请避开高峰期时段查询
						<br />
				...
				<div style="padding: 10px 10px 3px 10px">
					...
					<input type="text" name="code" class="html5input_n" style="width: 50%" onclick="clearMsg();">
					<img id="validateCode" alt="点击刷新"
						src="mobileT/edun?ac=getValidateCode&wxaccount=<%=wxaccount%>&t=<%=Math.random()%>"
						onclick="javascript:this.src='mobileT/edun?ac=getValidateCode&wxaccount=<%=wxaccount%>&t='+Math.random();"
						style="max-width: 48%; vertical-align: bottom; color: blue; font-size: 13px;">
					<hr />
					<input id="submit" type="button" value="提 交" class="html5btn" onclick="check();">
					<div id="msg"></div>
				</div>
			</div>
