...
.boxin {
	border-bottom: 1px solid #70B5FF;
	border-right: 1px solid #70B5FF;
	background-color: #FFF;
	padding: 10px;
}
...
.boxout:nth-child(odd) .boxin {
	border-left: 1px solid #70B5FF;
}

.boxout:first-child .boxin {
	border-top: 1px solid #70B5FF;
}
...
function search() {
	...
	<body onload="checkMM();" onclick="hideMenu(event);">
		<div
			...
			<div
				style="background-color: #70B5FF; text-align: center; color: #fff; padding: 5px 10px;">
				<span style="font-size: 20px;">校园二手交易平台</span>
				<br />
				<span style="font-size: 14px;">只为校园而生</span>
			</div>
		</div>
		<div>
			<%
			...
			<div class="boxout"
				...
				<div class="boxin">
					...
					<div class="price_old">
						...
						<%
							String[] olds = { "五成新", "六成新", "七成新", "八成新", "九成新", "全新" };
						%>
						<span style="float: right"><%=olds[goods.getOld() - 5]%></span>
					</div>
				</div>
			</div>
			<%
		...
		<%
			String url = "/mobile/bns?ac=list&wxaccount=" + wxaccount + "&cate=" + cate+"&type="+type;
		%>
