...
function check() {
	...
	var url = "/mobile/edu?ac=login&wxaccount=<%=wxaccount%>&userwx=<%=userwx%>";
	$
			.post(
					url,
					{
					...
					function(data) {
						...
						if (obj == "ok") {
							if('<%=ac %>' == 'course'){
								location.href = "/mobile/edu?ac=getAllCourse&wxaccount=<%=wxaccount%>&userwx=<%=userwx%>";
							}else if('<%=ac %>' == 'sport'){
								location.href = "/mobile/edu?ac=getAllSport&wxaccount=<%=wxaccount%>&userwx=<%=userwx%>";
							}
						} else {
							if (obj == "wrong") {
								obj = "网络连接失败";
