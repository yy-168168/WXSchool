...
public class ArticleMng extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (ac.equals("list")) {
			...
			int totalRecord = articleDao.getTotalRecordofArt(wxid);
			Page page = new Page(curPage, 10, totalRecord);

			List<Article> articles = articleDao.getArticlesofArt(wxid, page);

			request.setAttribute("page", page);
			request.setAttribute("articles", articles);
			request.getRequestDispatcher("/mng/articleList.jsp").forward(
					request, response);
			...
			request.getRequestDispatcher("/mng/articleAddorUpt.jsp?ac=uptArt")
					.forward(request, response);
		} else if (ac.equals("addArt_")) {
			request.getRequestDispatcher("/mng/articleAddorUpt.jsp?ac=addArt")
					.forward(request, response);
		...
		} else if (ac.equals("getArtCate")) {
			String[] cates = articleDao.getArticleCate(wxid);
			BackJs.backJs(JSONObject.toJSONString(cates), response);
		} else if (ac.equals("search")) {
			String kw = request.getParameter("keyword");

			Page page = new Page(1, 15, 15);
			List<Article> articles = articleDao.searchByKeywordofArt(wxid, kw,
					page);
